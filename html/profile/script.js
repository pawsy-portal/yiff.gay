...
async function load() {
    try {
        ...
        if(d.insígnias) d.insígnias.forEach(i => b.innerHTML += `<img src="${i.url}" class="bdg-i">`);
...
window.addEventListener('hashchange', () => { cur = window.location.hash.substring(1) || BASE; reset(); });
